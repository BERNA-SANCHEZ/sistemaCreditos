...
switch ($accion) {
	...
	case 'prestamosPendientesClientes':
		...
		break;
	case 'pagarPendientes':
			pagarPendientes();
		break;	
	case 'cambiarEstadosPrestamo':
			cambiarEstadosPrestamo();
		break;	
	case 'cambiarruta':
			cambiarruta();
		break;
...
function cambiarEstadosPrestamo(){
	...
	{	
		...
		if ( $procederPrestamo == 1 ) {
			...
			$detalle = preg_replace("/([a-zA-Z0-9_]+?):/" , "\"$1\":", $_REQUEST["det_prestamos_pendientes"]); // fix variable names 
			$arrayDetalle = json_decode($detalle, true); 	

			foreach ($arrayDetalle as $key => $value) {
				$sql = "UPDATE detprestamos SET tipo = 0 WHERE tipo = 3 AND idprestamo = {$value['id']} ";
				if($conexion->sqlOperacion($sql)["resultado"] == false){ $error++; }; 

				$sql = "UPDATE detprestamos SET morapagada = 0 WHERE morapagada = 4 AND abonomora = 0 AND idprestamo = {$value['id']} ";
				if($conexion->sqlOperacion($sql)["resultado"] == false){ $error++; }; 

				$sql = "UPDATE detprestamos SET morapagada = 3 WHERE morapagada = 4 AND abonomora > 0 AND idprestamo = {$value['id']} ";
				if($conexion->sqlOperacion($sql)["resultado"] == false){ $error++; }; 
			}
...
function mostrarRP()
{
	...
	{	
		...
		IFNULL( (SELECT (SUM(monto) - SUM(abono)) AS pendiente FROM detprestamos WHERE idprestamo = prestamos.id AND pagado = 0 AND tipo = 0 )  ,0)  AS pendiente
	...
	{	
		...
		foreach ($result as $key => $value1) {			
			...
			foreach ($detallepagos as $key => $value2) {
				...
				if ($value2["morapagada"] != 2 && $value2["morapagada"] != 4) {
					$sma += $value2["mora"];                    
				}
...
function mostrarDetPrestamo(){
	...
	{	
		...
		if ($buscarEstadoPrestamo[0]["estado"] == 1) {
			...
			$cuotasNoPagadas = $conexion->sql("SELECT id, fecha, morapagada FROM detprestamos 
			WHERE idprestamo = '{$_REQUEST['idprestamo']}' AND pagado = 0 
			AND fecha <= ADDDATE(CURDATE(),-1)");
			...
			if ($buscarplanesprestamo[0]["tipo"] == 1) {
				...
				foreach ($cuotasNoPagadas as $key => $value) {
					if ($value['morapagada'] == 1 || $value['morapagada'] == 2) {
						continue;
					}
					...
					if ($key == ($cantidadRegistros - 1)) {
						...
						if ($value['fecha'] == $ultimaFecha) {
							$fechahoy = date("Y-m-d", strtotime($value['fecha']." + 1 day"));
						}else{
							$fechahoy = date("Y-m-d");
						}	
		...
		$sql4="SELECT p.resumenpagos as cuota, pp.tipo, pp.cuotas, p.estado FROM prestamos p
	...
	{	
		...
		foreach ($detalle as $key => $valuex) {		
			...
			if ($buscarEstadoPrestamo[0]["estado"] == 1) {
				...
				#Buscamos las cuotas que no tiene pago y tampoco la mora ha sido pagada
				$cuotasNoPagadas = $conexion->sql("SELECT id, fecha, morapagada FROM detprestamos 
				WHERE idprestamo = '{$valuex['id']}' AND pagado = 0 AND fecha <= ADDDATE(CURDATE(),-1) ");
			...
			if ( $_REQUEST['btnPrimeroMorasPendientes'] == 1 && $_REQUEST['monto'] != '' && $buscarSumaMoras[0]["sumamoras"] > 0 ) {
				...
				if (count($buscarMorasPendientes) > 0){
					...
					if ($montoRecibido > 0) {
						...
						FROM detprestamos WHERE idprestamo = '{$_REQUEST['idprestamo']}' AND pagado = 0 AND tipo = 0");
						...
						if ($_REQUEST['monto'] <= $buscarMontoPendiente[0]["pendiente"] && $_REQUEST['monto'] != '') {
							...
							WHERE idprestamo = '{$_REQUEST['idprestamo']}' AND pagado = 0 AND tipo = 0 ORDER BY id ASC LIMIT {$cuotas} ");
							...
							if ($abono > 0) {
								$buscarCuotasPendiente = $conexion->sql("SELECT id FROM detprestamos 
								WHERE idprestamo = '{$_REQUEST['idprestamo']}' AND pagado = 0 AND tipo = 0 ORDER BY id ASC LIMIT 1 ");
...
function eliminarCapital_plan5(){
	...
	{	
		...
		WHERE idprestamo = '{$idprestamo}' AND pagado = 0 AND tipo = 0 AND abono = 0 AND mora = 0 OR monto = 0");
	...
	{	
		...
		if ( $procederPrestamo == 1 ) {
			...
			if ($_REQUEST['monto'] <= $totalPendiente ) {
				...
				//Solo se les cambia el monto a las cuotas que aun no tienen mora, y aun no llega la fecha de cobro.
				$buscardetprestamos = $conexion->sql("SELECT * FROM detprestamos 
				WHERE idprestamo = '{$_REQUEST['idprestamo']}' AND pagado = 0 AND tipo = 0 AND abono = 0 AND mora = 0");
				...
				if (count($buscarMorasPendientes) > 0){
					...
					if ($montoRecibido > 0) {
						...
						if ($_REQUEST['monto'] <= $buscarMontoPendiente[0]["pendiente"] && $_REQUEST['monto'] != '') {
							...
							WHERE idprestamo = '{$_REQUEST['idprestamo']}' AND pagado = 0 AND tipo = 0 ORDER BY id ASC");
...
function eliminarPago(){
	...
	{	
		...
		if ( $_REQUEST['pagocuota'] == 1 ) {
			...
			AND fecha <= ADDDATE(CURDATE(),-1) AND morapagada != 2 AND morapagada != 4");
		...
		if ( $_REQUEST['pagocuota'] == 5 && $_REQUEST['tipoPlan'] != 5 ) {
			...
			$buscarCuotasPendiente = $conexion->sql("SELECT id, monto FROM detprestamos 
			WHERE idprestamo = '{$idprestamo}' AND pagado = 0 AND tipo = 0 ORDER BY id ASC");
...
function mostrarMorasXprestamo(){
	...
	{	
		...
		AND fecha <= ADDDATE(CURDATE(),-1) AND (morapagada = 0 OR morapagada = 4) AND abonomora = 0 ORDER BY id ASC";			
...
function pagarMora(){
	...
	{	
		...
		if ( $procederPrestamo == 1 ){
			...
			'0' , '".utf8_decode("Pago por moras")."', '".$_REQUEST['montomora']."', '".$_REQUEST['fechaPagarMora']."' , '4','0','0','".$idapertura."' )");
			if ($regpagosrealizados['resultado'] == false) { $error++; }
...
function dejarPendienteCobro(){
	...
	{	
		...
		$buscarMoraExonerada = $conexion->sql("SELECT morapagada FROM detprestamos WHERE id = {$_REQUEST['id']} ");


		if ( $buscarMoraExonerada[0]["morapagada"] == 2) {
			$sql = "UPDATE detprestamos SET tipo = '3' WHERE id = {$_REQUEST['id']} ";
			if($conexion->sqlOperacion($sql)["resultado"] == false){ $error++; }; 
		}else{
			$sql = "UPDATE detprestamos SET morapagada = '4', tipo = '3' WHERE id = {$_REQUEST['id']} ";
			if($conexion->sqlOperacion($sql)["resultado"] == false){ $error++; }; 
		}
...
function pagarPendientes(){
	...
	{	
		...
		if ( $procederPrestamo == 1 ){
			...
			if ( $_REQUEST['valorMoraPendiente'] == 0 ) {
				$descripcion_de_pago = "Pago de Cuotas";
				$estadopagorealizado = 1; //1=Pago de Cuotas													
			}else{
				$valorabonocuota = $_REQUEST['newPendienteCantidad'] - $_REQUEST['valorMoraPendiente'];
			}
			...
			$regpagosrealizados = $conexion->sqlOperacion("INSERT INTO pagosrealizados (idprestamo, idcliente, idusuario, 
			idcierreganancias, iddeposito, plan, descripcion, monto, fechapago, estado, deposito, idtransaccion,abonocuota, idapertura) 
			VALUES ('".$_REQUEST['idprestamo']."','".$buscarDatoCliente[0]["idcliente"]."', '".$_SESSION["idusuario"]."', '0','0',
			'0' , '".utf8_decode($descripcion_de_pago)."', '".$_REQUEST['newPendienteCantidad']."', '".$_REQUEST['fechapendienteinicio']."' , 
			'".$estadopagorealizado."','0','0','".$valorabonocuota."','".$idapertura."' )");
			if ($regpagosrealizados['resultado'] == false) { $error++; }

			if ( $_REQUEST['valorMoraPendiente'] == 0 ) {
			
				$sql = "UPDATE detprestamos SET fechapago= '".$_REQUEST['fechapendienteinicio']."' , 
				abono = '0', pagado = '1', tipo = '0' WHERE id = {$_REQUEST['idDejarCuotaPendientx']} ";
				if($conexion->sqlOperacion($sql)["resultado"] == false){ $error++; }; 
				...
				$sql = "UPDATE detprestamos SET morapagada= '1', abonomora='0' 
				WHERE id = {$_REQUEST['idDejarCuotaPendientx']} ";
				if($conexion->sqlOperacion($sql)["resultado"] == false){ $error++; }; 				
			}
...
function cambiarruta(){
	...
	{	
		...
		$sql = "UPDATE prestamos SET idcobrador = '{$_REQUEST['rutaseleccionada']}' WHERE id = {$_REQUEST['idprestamoCambiar']} ";
		if($conexion->sqlOperacion($sql)["resultado"] == false){ $error++; }; 
